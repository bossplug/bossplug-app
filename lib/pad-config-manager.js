...
module.exports =  class PadConfigManager {
    ...
    async getSoundAtCell(cellId)
    {
    //  var cellId = cell.cellId;

      console.log('get sound at ',cellId )
    ...
    async assignAttributeToCellConfig(assignment)
    {
      ...
      var cellId = assignment.cellId;
      var name = assignment.attributeName;
      var val = assignment.value;
      var enabled = assignment.enabled;

    //  console.log('before',cellId, JSON.stringify(padConfig))

      if(padConfig != null && padConfig.cells != null
        && padConfig.cells[cellId].attributes )
      {
        //clone
        var originalData = JSON.parse(JSON.stringify(cellEditorConstants.cellAttributes[name]))


         originalData.enabled = enabled;
         originalData.value = val; //reserved ?

         padConfig.cells[cellId].attributes[name]  = originalData;


          // console.log('after',cellId, JSON.stringify(padConfig))
      ...
      return {success:false, message: "Error assigning " +  name + " to cell "+cellId }
    }
    ...
    async exportSFX()
    {
      ...
      var path = await StorageHelper.showOpenDialog({
      properties: ["openDirectory"]
    })



      var cellList = this.getPadCellList()
      ...
      for(var i in cellList)
      {
          ...
          var targetPath = path + '/' + cell.sfxName;
          var sourcePath = this.findFilePathUsingHash(cell.sfxHash);

          console.log('copying ' , sourcePath, 'to ', targetPath )

          await StorageHelper.copyFileSync(sourcePath,targetPath)

      }



      return {success:true,message:'Exported SFX.'}
    ...
    setPadConfig(config,msg)
    {
      ...
      if(msg == null)
      {
        ...
        this.deviceManager.updatePadConfig(padConfig)
    ...
    static populateCellWithAttributes(cell)
    {
      ...
      if(!cell.attributes) cell.attributes = {}
        ...
        for(var key in cellEditorConstants.cellAttributes)
        {
          var val;

          var attr = cellEditorConstants.cellAttributes[key]
