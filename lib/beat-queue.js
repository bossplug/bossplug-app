...
*/
    ...
    async preloadAudio( padConfigManager)
    {
      //console.log('beat preload audio ',audioFiles);

      var audioFiles = await  padConfigManager.getSelectedAudioFiles();
      ...
      await  padConfigManager.markAudioFilesPreloaded(bufferedAudioFiles)
    ...
    {
      if ( !fs.existsSync( appRoot+'/public/audio' ) ) {
          fs.mkdirSync( appRoot+'/public/audio' );
      }


      var dest = appRoot+'/public/audio/'+hash+'.wav';

      var exists = fs.existsSync(dest)

      console.log('exists?',exists,dest )
      if(!exists)
      {
