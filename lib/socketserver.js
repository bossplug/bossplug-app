...
module.exports =  class SocketServer {
  ...
  constructor(beatQueue,padConfigManager,deviceManager){
      ...
      io.on('connection', function (socket) {
        ...
        console.log('established new socket connection');
          ...
          socket.on('connectToLaunchpad', async function (data,fn) {
                var response = await self.deviceManager.connectToLaunchpad()
                console.log(response)
                fn(JSON.stringify(response));  //immediate response
          });

          socket.on('pressedKey', async function (e,fn) {
                var response = self.deviceManager.pressedKeyboard(e)
                fn(JSON.stringify(response));  //immediate response
          ...
          socket.on('queueSound', async function (sfx,fn) {
            ...
            socket.on('preloadAudio', async function (data,fn) {
                    ...
                    //var selectedAudioFiles = await self.padConfigManager.getSelectedAudioFiles();
                     ...
                     await self.beatQueue.preloadAudio( self.padConfigManager)
            ...
            socket.on('assignSoundToPadConfig', async function (assignment,fn) {
                    console.log('assignSoundToPadConfig', assignment)
                     await self.padConfigManager.assignSoundToPadConfig(assignment)
              ...
              socket.on('assignOptionToPadConfig', async function (assignment,fn) {
                ...
                //setting a cell name, for example
              socket.on('assignOptionToCellConfig', async function (assignment,fn) {
                      console.log('assignOptionToCellConfig', assignment)
                      var response = await self.padConfigManager.assignOptionToCellConfig(assignment)
                      fn(JSON.stringify(response));  //immediate response
                });

                //add a property like Pulse 
              socket.on('assignAttributeToCellConfig', async function (assignment,fn) {
                      console.log('assignAttributeToCellConfig', assignment)
                      var response = await self.padConfigManager.assignAttributeToCellConfig(assignment)
                      fn(JSON.stringify(response));  //immediate response
                });

              socket.on('getCellData', async function (cellId,fn) {
                    var response = await self.padConfigManager.getCellData(cellId)
                    fn(JSON.stringify(response));  //immediate response
              ...
              socket.on('loadPadConfig', async function (data,fn) {
                    await self.padConfigManager.loadPadConfig()
              ...
              socket.on('exportSFX', async function (data,fn) {
                    var response = await self.padConfigManager.exportSFX()
                    fn(JSON.stringify(response));  //immediate response
  ...
  async emit(name,value)
  {
    ...
    if(!self.socket)
    {
      console.log('socket does not exist ', self.socket)
      return {success:false,message:'Socket does not exist.'}
    }


    var response = await new Promise((resolve, reject) => {
      //  console.log('server emit',name,value)
