...
module.exports =  class DeviceManager {
    ...
    async pressedPad(x,y,pressed)
    {
      ...
      if(pressed)
      {
        var response = await this.activateCell({cellId:  i, activate:true});
        sfxConfig[i].pressed = true;
      }else{
        var response = await this.activateCell({cellId:  i, activate:false});
  ...
  /*  async setPadColor(k)
    ...
    */
    ...
    async activateCell(activation)
    {
      var sfx = await this.padConfigManager.getSoundAtCell(activation.cellId);
      console.log('activate sfx',sfx, activation)

      var response = await this.socketServer.emit('queueSFXEvent',{sfx:sfx, activate: activation.activate})
